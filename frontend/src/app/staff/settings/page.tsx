...
function ProfileTab({ user }: { user: ReturnType<typeof useAuthContext>["user"] }) {
  ...
  const save = async () => {
    ...
    setSaving(false);
    setMsg({ type: res.success ? "success" : "error", text: res.success ? "บันทึกสำเร็จ" : (res.message ?? "เกิดข้อผิดพลาด") });
  };
  ...
  return (
    <div className="space-y-4 max-w-md">
      ...
      <div className="space-y-3">
        {[
          { label: "เหตุฉุกเฉินใหม่", desc: "แจ้งเตือนเมื่อมีการแจ้งเหตุ" },
          { label: "ยาสต๊อกต่ำ", desc: "แจ้งเตือนเมื่อยาต่ำกว่าเกณฑ์" },
          { label: "ยาใกล้หมดอายุ", desc: "แจ้งเตือนล่วงหน้า 30 วัน" },
          { label: "การนัดหมายใหม่", desc: "แจ้งเตือนเมื่อมีการจองนัด" },
        ].map(({ label, desc }) => (
...
function ContactsTab() {
  ...
  const getCat = (cat: string) => CONTACT_CATEGORIES.find((c) => c.value === cat);
          ...
          {users.map((u) => (
            <div key={u.id} className="flex items-center gap-3 p-3 border border-gray-100 rounded-xl">
              ...
              <span className={cn(
                ...
                u.role === "staff" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"
              )}>
                {u.role === "admin" ? "ผู้ดูแล" : u.role === "staff" ? "เจ้าหน้าที่" : "นักศึกษา"}
              </span>
              {!u.isActive && (
                <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">ปิดการใช้งาน</span>
              )}
            </div>
          ))}
        </div>
...
function BackupTab() {
  ...
  const [msg, setMsg] = useState<{ type: "success" | "error"; text: string } | null>(null);
  const [backups, setBackups] = useState<Array<{ id: string; filename: string; status: string; fileSizeBytes: number | null; startedAt: string }>>([]);
              ...
              <div key={b.id} className="flex items-center gap-3 p-3 border border-gray-100 rounded-xl text-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {new Date(b.startedAt).toLocaleDateString("th-TH", { dateStyle: "medium" })}
                  </p>
                </div>
                ...
                <span className={cn(
                  ...
                  {b.status === "completed" ? "สำเร็จ" : b.status === "failed" ? "ล้มเหลว" : "กำลังดำเนินการ"}
                </span>
              </div>
            ))}
          </div>
