      ...
      <div className="bg-gradient-to-br from-blue-800 to-blue-950 px-5 pt-10 pb-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            {user?.studentId && (
              <p className="text-sm text-blue-300 mt-0.5">{user.studentId}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="relative w-10 h-10 flex items-center justify-center bg-white/15 rounded-xl"
            >
            ...
            <Link
              ...
              className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center font-bold text-white text-sm shadow-sm"
            >
              {initials}
            </Link>
          </div>
      ...
      <div className="px-4 pt-5 pb-6 space-y-5">
        ...
        {activeIncident && (
          <Link
            ...
            className="flex items-center gap-3 bg-amber-600 text-white rounded-2xl px-5 py-4 shadow-lg shadow-amber-600/30"
          >
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0">
              <Siren size={20} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-base font-bold">เหตุฉุกเฉินกำลังดำเนินการ</p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm px-6 pt-6 pb-6 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider mb-4">
          ...
          <button
            ...
            className={cn(
              "w-44 h-44 rounded-full mx-auto flex flex-col items-center justify-center gap-1.5",
              "bg-gradient-to-b from-red-600 to-red-800",
              "shadow-[0_8px_32px_rgba(185,28,28,0.5)]",
              "border-[5px] border-red-300",
              "active:scale-95 transition-transform duration-100",
              "emergency-pulse"
            )}
          >
            <AlertTriangle size={50} className="text-white" />
            <span className="text-white font-extrabold text-lg">แจ้งเหตุ</span>
            <span className="text-red-200 text-sm">ฉุกเฉิน</span>
          </button>

          <a
            href="tel:191"
            className="mt-5 inline-flex items-center gap-2 text-base text-red-600 font-semibold hover:text-red-700 transition-colors"
          >
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <Link
              key={href}
              href={href}
              className="bg-white dark:bg-gray-800 rounded-2xl p-4 flex flex-col items-center gap-2.5 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md active:scale-95 transition-all"
            >
              <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center", bg)}>
                <Icon size={24} className={fg} />
              </div>
              <span className="text-sm text-gray-700 dark:text-gray-200 font-semibold">{label}</span>
            </Link>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200 dark:border-gray-700">
            <h2 className="font-semibold text-gray-800 dark:text-gray-100 text-base">ประวัติล่าสุด</h2>
            <Link
              href="/student/history"
              className="text-sm text-blue-700 dark:text-blue-400 flex items-center gap-0.5 font-medium hover:underline"
            >
          ...
          {loadingVisits ? (
            ...
            <div className="py-10 text-center">
              <ClipboardList size={32} className="mx-auto text-gray-300 dark:text-gray-600 mb-2" />
              <p className="text-sm text-gray-500 dark:text-gray-400">ยังไม่มีประวัติการใช้บริการ</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {recentVisits.map((visit) => (
                <div key={visit.id} className="px-5 py-3.5 flex items-center gap-3">
                  <div
                    className={cn(
                  ...
                  >
                    {visit.status === "completed" ? (
                      <CheckCircle2 size={18} className="text-green-700 dark:text-green-400" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">{formatDateTime(visit.createdAt)}</p>
                  </div>
                  <span
                    className={cn(
        ...
        {notifications.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200 dark:border-gray-700">
              <h2 className="font-semibold text-gray-800 dark:text-gray-100 text-base">การแจ้งเตือน</h2>
              <Link
                href="/student/notifications"
                className="text-sm text-blue-700 dark:text-blue-400 flex items-center gap-0.5 font-medium hover:underline"
              >
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {notifications.slice(0, 3).map((n) => (
                <div
                  key={n.id}
                  className={cn("px-5 py-3.5 flex gap-3 items-start", !n.isRead && "bg-blue-50/50 dark:bg-blue-950/20")}
                >
                  <div
                    className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{formatDateTime(n.createdAt)}</p>
                  </div>
                </div>
              ))}
            </div>
